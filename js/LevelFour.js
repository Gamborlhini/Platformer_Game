...
function winRedirect() {
  if (winblock == true) {
    location.assign("../html/LevelFive.html");
  }
}
function shotDetect(enemy) {
  ...
  if (this.x < enemy.x + enemy.width &&
    ...
    enemyhit = true;
    splat.play();
    setTimeout(function(){ splat.pause(); }, 1000);
  }
}
...
function drawNew() {
  ...
  ctx.fillStyle = "black";
  ctx.font = "20px Arial"
  ctx.fillText(`Press F to shoot the enemy`, 20, 30)
  ctx.fillText('The enemy will follow you if you are in its range', 20, 80)
  ctx.fillText('Press R to reload', 20, 55)
  ctx.fillText('You will be damaged if you are touching the enemy', 20, 105)
  //  if (direction == "right") {
